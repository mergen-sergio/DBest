...
However, this is just **one possible way** to incorporate the `movie` table. Other alternatives exist, such as performing a final join with the `movie` table.  We demonstrated this version using the **Reference operator** to showcase the diversity of possible operations and representations.  
