...
<img src="assets/images/not in vs exists 1.png" alt="Expressing IN and EXISTS subqueries" width="800"/>
...
### Handling `NOT IN` vs. `NOT EXISTS` in DBest  


The previous example demonstrated a case where a query could be expressed using either `NOT IN` or `NOT EXISTS`. However, this is not always possible. If the list produced by the `NOT IN` subquery contains `NULL` values, the query will return an **empty result set** due to SQL's **NULL semantics**.  
...
<img src="assets/images/not in 1.png" alt="Expressing NOT IN with DBest" width="900"/>  
